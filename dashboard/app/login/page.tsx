      ...
      <div className="w-full max-w-md p-8">
        <div className="text-center mb-10">
          ...
          <p className="mt-2 text-sm" style={{ color: 'var(--text-muted)' }}>
            Sign in with your Covr Google account
          </p>
        </div>

        <button
          onClick={() => signIn('google', { callbackUrl: '/' })}
          className="w-full flex items-center justify-center gap-3 font-medium py-3 px-4 rounded-lg transition-all duration-200"
          style={{
        ...
        <p className="text-[11px] text-center mt-5" style={{ color: 'var(--text-muted)' }}>
          Only @covr.care email addresses are allowed
        </p>
      </div>
    </div>
  );
